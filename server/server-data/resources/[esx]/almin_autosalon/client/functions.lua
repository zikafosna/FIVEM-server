...
OpenGarageMenu = function()
    ...
    ESX.TriggerServerCallback("garage:fetchPlayerVehicles", function(fetchedVehicles)
        ...
        if #menuElements == 0 then
            table.insert(menuElements, {
                ["label"] = "Nemate ni jedno vozilo u ovoj garaži."
            })
    	...
    	TriggerEvent("pNotify:SendNotification", {text = "Recite ljudima u blizini da se pomere!", type = "success", queue = "success", timeout = 2500, layout = "center"})
    end
end
...
OpenVehicleMenu = function()
    ESX.TriggerServerCallback("garage:fetchPlayerVehicles", function(fetchedVehicles)
            ...
            for _, vehicle in ipairs(gameVehicles) do
                if DoesEntityExist(vehicle) then
                    ...
                    if Config.Trim(GetVehicleNumberPlateText(vehicle)) == Config.Trim(vehicleProps["plate"]) then
                        table.insert(menuElements, {
                            ["label"] = GetLabelText(GetDisplayNameFromVehicleModel(vehicleProps["model"])) .. " | " .. vehicleData["plate"] .. " - " .. dstCheck .. " metara.",
        ...
        if #menuElements == 0 then
            table.insert(menuElements, {
                ["label"] = "Nemate ni jedno vozilo, van garaže"
            })
        end
...
ChooseVehicleAction = function(vehicleEntity, callback)
    ...
    ESX.UI.Menu.Open("default", GetCurrentResourceName(), "second_vehicle_menu", {
        ["title"] = "Upravljaj Vozilom | " .. GetVehicleNumberPlateText(vehicleEntity),
        ["align"] = Config.AlignMenu,
        ["elements"] = menuElements
    }, function(menuData, menuHandle)
...
VehicleAction = function(vehicleEntity, action)
    ...
    if action == "change_lock_state" then
        if dstCheck >= Config.RangeCheck then return ESX.ShowNotification("Previše ste udaljeni od ~b~vozila~s~ da biste ga kontrolisali.") end
        ...
        ESX.ShowNotification("Vozilo | ~b~" .. GetVehicleNumberPlateText(vehicleEntity) .. "~s~ je sada " .. (vehicleLockState == 1 and "~r~zakljucano." or "~g~otkljucano."))
...
SpawnLocalVehicle = function(vehicleProps)
    local spawnpoint = Config.Garages[cachedData["currentGarage"]]["positions"]["izguraj"]
    ...
    if not ESX.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then
        ESX.ShowNotification("Pomerite vozilo koje blokira izlaz. [1]")
...
SpawnVehicle = function(vehicleProps)
	...
	if not ESX.Game.IsSpawnPointClear(spawnpoint["position"], 3.0) then 
		ESX.ShowNotification("Pomerite vozilo koje blokira izlaz. [2]")
	...
	for i = 1, #gameVehicles do
        ...
        if DoesEntityExist(vehicle) then
			if Config.Trim(GetVehicleNumberPlateText(vehicle)) == Config.Trim(vehicleProps["plate"]) then
				ESX.ShowNotification("Vaše vozilo je van garaže, ne možete izvaditi ista dva vozila.")
	...
	ESX.Game.SpawnVehicle(vehicleProps["model"], spawnpoint["position"], spawnpoint["heading"], function(yourVehicle)
        ...
        ESX.ShowNotification("Isparkirali ste vozilo.")
	...
	if DoesEntityExist(vehicle) then
		...
		ESX.TriggerServerCallback("garage:validateVehicle", function(valid)
			if valid then
				...
				ESX.ShowNotification("Možete parkirate samo ~b~vaše vozilo~s~.")
			end
...
WaitForModel = function(model)
	...
	while not HasModelLoaded(model) do
		...
		DrawScreenText("SACEKAJTE! Ucitavanje modela...", 255, 255, 255, 150)
	end
end