$(function(){
	...
	$('.users').on('click', '.spec', function(){
		...
		if (target == player) {
			...
			$('.spectate').fadeOut();
			$.post('http://esx_spec/select', JSON.stringify({id: target}));
		}
	...
	$('.users').on('click', '.kick', function(){
		...
		if (reason == null || reason == "") {
		    ...
		    $('.spectate').fadeOut();
			$.post('http://esx_spec/kick', JSON.stringify({id: target, reason: reason}));
		}
	});

	$('.header').on('click', '#close', function(){
		$('.spectate').fadeOut();
		$.post('http://esx_spec/quit');
	});
